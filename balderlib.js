...
function triangle(x1, y1, x2, y2, x3, y3, color, lineWidth) {
    context.beginPath();
    context.moveTo(x1, y1);
    context.lineTo(x2, y2);
...
function text(value, x = 0, y = canvas.height, fontSize = 24, color) {
    if (fontSize)
...
// 
class Turtle {
    constructor(x = canvas.width / 2, y = canvas.height / 2, direction = 0, isPenDown = true) {
        this.x = x;
        this.y = y;
...
class Cell {
    ...
    set char(char) {
        this.fill();
        if (char) {
            text(char[0], this.x + this.width / 2 - this.charWidth / 2, this.y + this.height / 2 + this.charHeight / 2, this.fontSize, this.foreColor);
            this._char = char[0];
        }
    }
    fill(color) {
...
function createColor(red, green, blue, alpha = 1) {
